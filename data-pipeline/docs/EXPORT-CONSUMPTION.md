...
| `normalized/<productKey>.json` → `rxnorm` block | `npm run content:sync` derives `src/sources/content/coverage/product-concepts.json` (exact SBD + generic SCD per product) | `src/patient/lib/coverage/concepts.ts` |
