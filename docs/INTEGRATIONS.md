...
The archive is ~2.2 GB. The data-pipeline package fetches only the members it
needs by HTTP range request (~9 MB), extracts the rows for every product's
brand and generic RxNorm concepts plus the plan index, and `content:sync`
...
that; 39 plans in this release share the name "AARP Medicare Rx Preferred from
UHC (PDP)", so a name is never treated as an identity. A typed name falls back
to conservative token matching and refuses below the threshold.

**Brand versus generic.** Each product is searched by its exact branded
concept first, then its generic clinical drug of the same strength and form.
The result reports which was found (`matchGranularity`), and a generic-only
hit says so in the headline.
