...
The archive is ~2.2 GB, so only the extracted subset ships. That subset IS
committed, because the app reads it at runtime and a Vercel build has neither
network access to CMS nor a pipeline checkout:
...
the rows matching the catalogue's RXCUIs plus the plan index.

`npm run coverage:ingest` is the older standalone ingester. It reads RXCUIs from
a single hardcoded source record, so it produces a ONE-product snapshot and now
refuses to overwrite the wider one without `--force`.
