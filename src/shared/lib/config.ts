 ...
 *
 * `getAppMode()` above only decides where "/" sends someone. That left the
 * clinician workspace fully reachable at /doctor on the patient domain, which
 * is what the UI audit recorded as F-06: a patient following a shared link
 * could reach a screen built for a prescriber, containing prescriber-directed
 * labeling and a share tool, with nothing marking it as not for them.
 *
 ...
 *
 * Gating is a 404, not a redirect: "this deployment does not have that page"
 * is true, and a redirect would imply the page exists somewhere the reader can
 * get to, which for a clinician tool is not something to advertise.
 */
